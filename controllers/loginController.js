...
module.exports = {
        ...
        try {
            ...
            if (customerDataArray && customerDataArray.length > 0) {
                ...
                if (applicantMobileNo && customerNumber) {
                    ...
                    // bypass otp
                    const otp = (mobileNumber == process.env.BYPASS_MOBILE) ? process.env.TESTING_OTP_UAT : generateRandomOtp()

                    //generating random unique id
                    const query = `SELECT * FROM user_data WHERE mobile_number = ?`;
                    const [userDataRows] = await pool.promise().execute(query, [applicantMobileNo]);
                    let uid
                    ...
                    if (mobileNumber == process.env.BYPASS_MOBILE) {
                        ...
                        if (otpResponse.success) {
                            ...
                            res.json({ success: true, status: 200, message: "OTP sent successfully!!", data: { token: otpToken, otp: otp } });

                        } else {
                            res.status(500).json({ success: false, status: 500, message: "Failed to send OTP", data: {} });
                        }
                    }

                } else {
                    res.status(401).json({ success: false, status: 401, message: "User not exists", data: {} });
                }
            } else {
                res.status(200).json({
                    success: false,
    ...
    otpVerify: async (req, res) => {
        try {
            ...
            const otpToken = req.headers.authorization && req.headers.authorization.split(" ")[1];
            ...
            if (lang !== 'en') {
                ...
                jwt.verify(otpToken, process.env.SECRET_KEY, async (err, decoded) => {
                    ...
                    if (randomOtp && mobileNumber) {
                        ...
                        const authToken = jwt.sign({ mobileNumber }, process.env.SECRET_KEY, {
                            expiresIn: "2d",
                        });
    ...
    dobLogin: async (req, res) => {
        try {
			...
			console.log({ url: thirdPartyApi.getCustomerDetailsUsingMobile.endpoint,
               method: thirdPartyApi.getCustomerDetailsUsingMobile.method,
               headers: thirdPartyApi.getCustomerDetailsUsingMobile.headers,
			data: requesData})
            const result = await apiFetcher({
            ...
            if (customerDataArray && customerDataArray.length > 0) {
                ...
                if (applicantMobileNo && customerNumber) {
					...
					console.log(loanAccountNumber === BYPASS_LOAN_ACCOUNT &&
						dob === BYPASS_DOB, loanAccountNumber, BYPASS_DOB, dob)
					let otp;

					if (
						loanAccountNumber === BYPASS_LOAN_ACCOUNT &&
						dob === BYPASS_DOB
					) {
					...
					const shouldSendOtp = !(loanAccountNumber === BYPASS_LOAN_ACCOUNT && dob === BYPASS_DOB);
    ...
    dobLoginUpdated: async (req, res) => {
        try {
            ...
            if (!customerData || customerData.status === 'Failure' || customerData.status === 'failed') {
                return res.status(200).json({
                    success: false,
                    status: 200,
            ...
            if (dobThirdPartyResponseArray && dobThirdPartyResponseArray.length > 0) {
                ...
                if (isUAT) {
                    ...
                    const devMobileNumber = ['7030528541', '9763798262', '7003765564', '7003092937', '9830440111']
